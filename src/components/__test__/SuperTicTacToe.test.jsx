...
describe("SuperTicTacToe", () => {
  ...
  it("Reset button should reset the game", async () => {
    render(<SuperTicTacToe />)
    const boards = screen.getAllByRole("tic-tac-toe-board")
    const squares = Array(9).fill(null)
    for (let i=0; i<squares.length; i++)
    ...
    boards.forEach(board => {
      expect(within(board).queryByText(TURNS.X)).toBeNull()
      expect(within(board).queryByText(TURNS.O)).toBeNull()
    })
  })
  
  it("Should show when there's a draw in a board", async () => {
    render(<SuperTicTacToe />)
    const boards = screen.getAllByRole("tic-tac-toe-board")
    const squares = Array(9).fill(null)
    for (let i=0; i<squares.length; i++)
    ...
    await userEvent.click(squares[4][2])
    await userEvent.click(squares[2][4])
    await userEvent.click(squares[4][3])
    await userEvent.click(squares[3][4])
    await userEvent.click(squares[4][7])
    await userEvent.click(squares[7][4])
    await userEvent.click(squares[4][4])
    await userEvent.click(squares[4][1])
    await userEvent.click(squares[1][4])
    await userEvent.click(squares[4][5])
    await userEvent.click(squares[5][4])
    await userEvent.click(squares[4][6])
    await userEvent.click(squares[6][4])
    await userEvent.click(squares[4][8])
    screen.getByText("-")
  })
})
